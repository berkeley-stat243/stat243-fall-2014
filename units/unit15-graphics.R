...
par(new = TRUE)  # this causes next call to plot to overplot instead of replacing the original plot
plot(x, z, col = 'red', bty = 'n', ann = FALSE, xaxt = 'n', yaxt = 'n') # plot z points in red
...
plot(x, y, col = groups)
legend(-.5, 1, legend = c("Stats", "PoliSci", "Math"), col = 1:3, pch = 1, bty = "n") # change the first two arguments (the x,y coords of the upper left of the legend) to position the legend in a different place
plot(x, y, pch = groups)
legend(locator(), legend = c("Stats", "PoliSci", "Math"), pch = 1:3) # right-click to position legend manually
...
thetaVal <- 1.23
mtext(bquote(hat(theta) == .(thetaVal)), side = 3, line= 1.5) # put text on top, 1.5 lines out 
mtext(substitute(paste("Estimate: ", hat(theta) == thetaVal), list(thetaVal = thetaVal)), side = 1, line= 2)  # this puts the text on the bottom, 2 "lines" out
...
dev.off()
split.screen(figs = matrix(c(0, .4, 0, .5,  # xl, xu, yl, yu values to position the first subplot
...
library(fields) # includes image.plot(), which takes image() and adds a legend
...
n <- 16
pie(rep(1, n), col = rainbow(n)) # rainbow varies hue while keeping s and v constant
